...
────────────────────
Reads the "Bootcamp Schedule" tab from your Google Sheet and generates
bootcamp_day1.html … bootcamp_day5.html (plus an index redirect).
...
Auth:
  First run: opens a browser for one-time Google OAuth consent.
  After that: token is cached in ~/.config/checkr_bootcamp/token.json
  You only need to re-auth if you revoke access or the token expires (~6 months).
"""
